...
onMounted(async () => {
  ...
  // Busca a role no backend e salva na store
  try {
...
<template>
  <!-- Fundo global com cor levemente acinzentada para destacar os painéis -->
  <div class="h-screen flex bg-[#f1f5f9] overflow-hidden">
    ...
    <aside
      ...
      class="fixed top-4 bottom-4 left-4 z-50 w-64 bg-[#19341a] text-white transition-transform duration-300 ease-in-out md:hidden rounded-2xl shadow-2xl flex flex-col"
    >
    ...
    <aside
      class="hidden md:flex w-64 flex-shrink-0 m-4 mr-2 bg-[#19341a] text-white rounded-2xl shadow-xl flex-col"
    >
    ...
    <div class="flex flex-col flex-1 overflow-y-auto">
      <!-- Topbar Flutuante -->
      <header
        class="flex items-center justify-between h-16 px-4 md:px-6 my-4 mx-2 md:mx-4 bg-white rounded-2xl shadow-sm border border-gray-100 z-10"
      >
      ...
      <!-- Conteúdo da Página (Flutuante) -->
      <main
        class="flex-1 mx-2 mb-4 md:mx-4 md:mb-4 p-6 md:p-8 bg-white rounded-2xl shadow-sm border border-gray-100 overflow-y-auto"
      >
        <slot></slot>
      </main>
    </div>
