...
// Mock do usuário logado (Mude para 'colaborador' para testar a segurança)
const loggedUserRole = ref('admin')
// Simulação do ID do usuário logado (para evitar auto-exclusão no front-end)
...
const fetchData = async () => {
    ...
    try {
        ...
        // Dica: Quando integrar com o Supabase Auth real, 
        // pegue o ID do usuário logado e jogue em loggedUserId.value aqui.
        // Ex: loggedUserId.value = supabase.auth.user().id
...
const salvarMembro = async () => {
    ...
    try {
        ...
        toast.success('Membro adicionado com sucesso! Um e-mail foi enviado para definir a senha.')
    ...
    <Layout title="Membros e Acessos">
        <header class="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
                ...
                <p class="text-gray-500 text-sm mt-1">Gerencie os acessos e permissões do seu escritório.</p>
            </div>

            <button v-if="loggedUserRole === 'admin'" @click="isModalOpen = true"
            ...
            <div v-else class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            <th class="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase">Usuário</th>
                            <th class="px-6 py-4 text-center text-xs font-semibold text-gray-500 uppercase">Nível de Acesso</th>
                            <th v-if="loggedUserRole === 'admin'" class="px-6 py-4 text-right text-xs font-semibold text-gray-500 uppercase">Ações</th>
                        </tr>
                    </thead>
                    <tbody class="bg-white divide-y divide-gray-200">
                        <tr v-for="membro in membros" :key="membro.id" class="hover:bg-gray-50 transition-colors">
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    <!-- Avatar com cores do sistema -->
                                    <div class="h-10 w-10 rounded-full bg-[#19341a]/10 flex items-center justify-center text-[#19341a] font-bold uppercase shadow-sm">
                            ...
                            <td class="px-6 py-4 text-center">
                                <span class="px-2.5 py-1 rounded-full text-[11px] font-bold uppercase tracking-wider border" :class="getRoleBadge(membro.role)">
                                    {{ formatRole(membro.role) }}
                                </span>
                            </td>
                            <!-- Lógica: Só mostra o botão excluir se for admin E se não for o próprio usuário logado -->
                            <td v-if="loggedUserRole === 'admin'" class="px-6 py-4 text-right">
                                <button v-if="membro.id !== loggedUserId" @click="removerMembro(membro)" class="text-xs font-bold text-red-500 hover:text-red-700 hover:underline transition-colors">
                                    Excluir
                                </button>
            ...
            <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
                ...
                <div class="p-6 space-y-4">
                    <div>
                        ...
                        <input v-model="novoMembro.name" type="text" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#ff8a65]" />
                    </div>
                    <div>
                        <label class="block text-xs font-bold text-gray-500 uppercase mb-1">E-mail</label>
                        <input v-model="novoMembro.email" type="email" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#ff8a65]" />
                    </div>
                    <div>
                        <label class="block text-xs font-bold text-gray-500 uppercase mb-1">Nível de Acesso</label>
                        <select v-model="novoMembro.role" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm bg-white focus:outline-none focus:ring-2 focus:ring-[#ff8a65]">
                ...
                <div class="px-6 py-4 border-t border-gray-100 flex justify-end gap-2 bg-gray-50">
                    <button @click="isModalOpen = false" class="px-4 py-2 border text-gray-600 rounded-xl hover:bg-gray-100 text-sm font-semibold">Cancelar</button>
                    <button @click="salvarMembro" class="px-5 py-2 bg-[#ff8a65] hover:bg-[#f07047] text-white rounded-xl text-sm font-semibold shadow-sm">Salvar Membro</button>
                </div>
            </div>
        </div>
    </Layout>
